        ...
        <div class="container px-6 mx-auto grid pt-6">
            ...
            <div class="bg-white rounded shadow overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tr v-for="trainers in trainers.data" :key="trainers.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                        <td class="border-t">
                            <inertia-link class="px-6 py-4 flex items-center focus:text-indigo-500" :href="route('back.trainers.show', trainers.id)">
                                {{ trainers.name }}<br/>
                            </inertia-link>
                        </td>
                        <td class="border-t">
                            <inertia-link class="px-6 py-4 flex items-center" :href="route('back.trainers.show', trainers.id)" tabindex="-1">
                                <div v-if="trainers.phone">
                                    {{ trainers.phone }}
                                </div>
                            </inertia-link>
                        </td>
                        ...
                        <td class="border-t">
                            <inertia-link class="px-6 py-4 flex items-center" :href="route('back.trainers.show', trainers.id)" tabindex="-1">
                                <span v-if="trainers.company">{{ trainers.company.name }}</span>
                            </inertia-link>
                        </td>
                        <td class="border-t w-px">
                            <inertia-link class="px-4 flex items-center" :href="route('back.trainers.show', trainers.id)" tabindex="-1">
