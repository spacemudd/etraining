...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <p>Individual Attendance Report - Gradient Design</p>
        </div>
        
        <!-- Employee Info -->
        <div class="employee-info">
            <div class="employee-details">
                <div class="employee-name">{{ $active_trainees->first()->trainee->name_ar }}</div>
                <div class="employee-subtitle">{{ $active_trainees->first()->trainee->name_en }}</div>
                <div class="employee-id">رقم الهوية: {{ $active_trainees->first()->trainee->national_id }}</div>
            </div>
            @if ($base64logo)
                <img src="{{ $base64logo }}" alt="Company Logo" style="max-width: 80px; height: auto; border-radius: 10px; border: 3px solid rgba(255,255,255,0.3);">
            @endif
        </div>
        ...
        <div class="report-details">
            ...
            <div class="detail-card">
                ...
                <div class="detail-value">
                    @if ($active_trainees->first()->active)
            ...
            <thead>
                <tr>
                    ...
                    @if ($with_attendance_times)
                        ...
                        <th style="width: 120px;">وقت الانصراف</th>
                    @endif
            ...
            <tbody>
                @foreach ($days as $day)
                    ...
                    <tr>
                        ...
                        <td>
                            @if ($day['vacation_day'])
                                <span class="attendance-mark vacation">عطلة</span>
                            @elseif ($attendance)
                                @if ($attendance->status === 'present')
                                    <span class="attendance-mark present">حاضر</span>
                                @elseif ($attendance->status === 'absent')
                                    <span class="attendance-mark absent">غائب</span>
                                @elseif ($attendance->status === 'vacation')
                                    <span class="attendance-mark vacation">إجازة</span>
                                @endif
                            @else
                                <span class="attendance-mark absent">غائب</span>
                            @endif
                        ...
                        @if ($with_attendance_times)
                            <td>
                                @if ($attendance && $attendance->status === 'present' && $attendance->check_in)
                            ...
                            <td>
                                @if ($attendance && $attendance->status === 'present' && $attendance->check_out)
        ...
        <div class="summary-stats">
            @php
                ...
                foreach ($days as $day) {
                    if ($day['vacation_day']) {
                        ...
                        $attendance = $active_trainees->first()->trainee->attendances()
                            ->where('date', $day['date'])
                            ->first();
            ...
            <div class="stat-item">
                ...
                <div class="stat-label">أيام الإجازة</div>
            </div>
        </div>
        ...
        <div class="footer">
            ...
            <p><strong>نوع القالب:</strong> التصميم المتدرج الجديد - تقرير فردي</p>
