<template>
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <!-- Empty State -->
        <div v-else-if="!audits.length" class="p-8 text-center">
            <div class="inline-flex items-center justify-center w-12 h-12 mb-4 bg-gray-100 rounded-full">
                <svg class="w-6 h-6 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                </svg>
            </div>
            <p class="text-gray-500 font-medium mb-1">{{ $t('words.no-audit-records') || 'No audit records found' }}</p>
            <p class="text-sm text-gray-400">{{ $t('words.no-audit-records-description') || 'There are no changes recorded for this trainee yet.' }}</p>
        </div>

        <!-- Audit Records -->
        <div v-else class="divide-y divide-gray-200">
            <div v-for="(audit, key) in audits" :key="audit.id" 
