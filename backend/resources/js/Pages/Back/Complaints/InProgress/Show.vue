        ...
        <div class="container px-6 mx-auto grid pt-6">
            ...
            <breadcrumb-container
                :crumbs="[
                        ...
                        {title: 'in_progress_complaints', link: route('complaints.InProgress.Show')},
                    ]"
            ></breadcrumb-container>
                        ...
                        <tr v-for="trainees_complaint in trainees_complaints.data" :key="trainees_complaint.id"
                            ...
                            <td class="rtl:text-right text-black">
                                <button @click="RollOut(trainees_complaint.id)"
                                        type="button"
                                        v-if="trainees_complaint.complaints_status === 1"
                                        class="inline-flex items-center px-4 py-2 border border-transparent rounded-md font-semibold text-xs text-white uppercase ltr:tracking-widest focus:outline-none focus:shadow-outline-gray transition ease-in-out duration-150 disabled:cursor-not-allowed mx-2 font-bold bg-gray-700 hover:bg-red-600 active:bg-red-700 foucs:bg-red-700">
                                    {{ $t('words.done') }}
                                </button>
                            </td>
                        </tr>
    ...
    watch: {
        form: {
            handler: throttle(function() {
                ...
                this.$inertia.replace(this.route('complaints.InProgress.Show', Object.keys(query).length ? query : { remember: 'forget' }))
            }, 150),
            deep: true,
        },
    },
