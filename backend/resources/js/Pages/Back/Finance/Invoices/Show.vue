                ...
                <div class="w-full md:w-4/12" v-if="invoice.status === 0" v-can="'edit-invoice-amount'">
                    <div class="mx-5">
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                <h1 class="mb-8 font-bold text-xl">{{ $t('words.edit-amount')}}</h1>
                            </div>
                            <div>
                                <button
                                    v-if="!editButton.editOption"
                                    @click="editTrainee"
                                    class="items-center justify-end rounded-md px-4 py-2 bg-gray-200 hover:bg-gray-300 float-left text-right"
                                >
                                    {{ editButton.text }}
                                </button>
                                <button
        ...
        editTrainee() {
                if (!this.editButton.editOption) {
                    ...
                    if (confirm(this.$t('words.are-you-sure-to-delete'))) {
                        ...
                        axios.put(route('back.finance.invoices.update', this.invoice.id), newForm).then(response => {
                            Inertia.reload().then(() => {
                                this.editButton.editOption = false;
                                this.editButton.inputClass = 'mt-1 block w-full bg-gray-200';
                                this.editButton.selectInputClass = 'mt-1 block w-full border border-gray-200 bg-gray-200 py-2.5 px-4 pr-8 rounded leading-tight focus:outline-none';
