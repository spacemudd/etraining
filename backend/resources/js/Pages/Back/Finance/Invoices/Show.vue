<template>
    <app-layout>
        <div class="container px-6 mx-auto grid pt-6">
            ...
            <!-- Edit Amount Section -->
            <div class="mt-8 flex flex-col md:flex-row md:space-x-5 mt-8 mb-8" v-if="invoice.status === 0 && ($page.props.user.email === 'ebrahim.hosny@hadaf-hq.com' || $page.props.user.email === 'acc2@jisr-ksa.com' || $page.props.user.email === 'admin@admin.com')" v-can="'edit-invoice-amount'">
                <div class="w-full md:w-8/12">
            ...
            <div class="mt-8 flex flex-col md:flex-row md:space-x-5 mt-8 mb-8" v-if="$page.props.user.email === 'ebrahim.hosny@hadaf-hq.com' || $page.props.user.email === 'acc2@jisr-ksa.com' || $page.props.user.email === 'admin@admin.com'">
                <div class="w-full md:w-8/12">
                    <div class="grid grid-cols-2 gap-6">
                        <div>
                            <h1 class="mb-8 font-bold text-2xl">{{ $t('words.edit-company') }}</h1>
                        </div>
                        <div>
                            <button
                                v-if="!editCompanyButton.editOption"
                                @click="editCompany"
                                class="items-center justify-end rounded-md px-4 py-2 bg-blue-200 hover:bg-blue-300 float-left text-right"
                            >
                                {{ editCompanyButton.text }}
                            </button>
                            <button
                                v-else
                                @click="saveCompany"
                                :disabled="$wait.is('UPDATING_COMPANY')"
                                :class="{'bg-green-200 cursor-wait': $wait.is('UPDATING_COMPANY')}"
                                class="items-center justify-end rounded-md px-4 py-2 mx-2 bg-green-300 hover:bg-green-400 float-left text-right"
                            >
                                <svg v-if="$wait.is('UPDATING_COMPANY')" role="status" class="inline w-4 h-4 text-black animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="#E5E7EB"/>
                                    <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentColor"/>
                                </svg>
                                {{ editCompanyButton.text }}
                    ...
                    <div class="white-bg rounded bg-gray-50 rounded shadow-lg p-5">
                        <div class="col-span-6 sm:col-span-4 px-8">
                            <jet-label
                                for="company"
                            ...
                            <company-search-select
                                ...
                                :disabled="!editCompanyButton.editOption"
                                placeholder="ابحث عن شركة جديدة..."
                                @change="onCompanyChange"
                            />
                            <div v-if="editCompanyButton.editOption && selectedCompany" class="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                                <div class="text-sm text-green-800">
                                    <strong>الشركة الحالية:</strong> {{ invoice.company ? invoice.company.name_ar : 'غير محدد' }}
                                </div>
                                <div class="text-sm text-green-700 mt-1">
                                    <strong>الشركة الجديدة:</strong> {{ selectedCompany.name_ar }}
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    methods: {
        ...
        editInvoice() {
            if (!this.editButton.editOption) {
                ...
                if (confirm(this.$t('words.are-you-sure-to-edit'))) {
                    this.$wait.start('UPDATING_TRAINEE');
                    this.validationErrors = null;
                    this.$inertia.put(route('back.finance.invoices.update', this.invoice.id), newForm, {
                        preserveScroll: true,
                        onSuccess: () => {
                            this.editButton.editOption = false;
                            this.editButton.inputClass = 'mt-1 block w-full bg-gray-200';
        ...
        editCompany() {
            if (!this.editCompanyButton.editOption) {
                this.editCompanyButton.editOption = true;
                this.editCompanyButton.text = this.$t('words.save');
        ...
        saveCompany() {
            ...
            if (confirm(`هل أنت متأكد من تغيير الشركة من "${this.invoice.company ? this.invoice.company.name_ar : 'غير محدد'}" إلى "${this.selectedCompany.name_ar}"؟`)) {
                ...
                this.$inertia.put(route('back.finance.invoices.update-company', this.invoice.id), {
                    ...
                    onStart: () => {
                        this.$wait.start('UPDATING_COMPANY');
                    },
                    onFinish: () => {
                        this.$wait.end('UPDATING_COMPANY');
                        this.editCompanyButton.editOption = false;
                        this.editCompanyButton.text = this.$t('words.edit');
