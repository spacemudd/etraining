                ...
                <form @submit.prevent="submitForm">
                    ...
                    <div class="mt-4">
                        ...
                        <label class="mt-5 mx-auto w-64 flex flex-col items-center px-4 py-6 bg-white text-blue rounded-lg shadow-lg border border-blue cursor-pointer hover:bg-blue hover:text-red-500">
                            ...
                            <span class="mt-2 text-base leading-normal" v-if="!files.cv_full">{{ $t('words.select-file') }}</span>
                            <span class="mt-2 text-base leading-normal" v-else>{{ this.files.cv_full.name }}</span>
                            <input type='file'
                                   class="hidden"
                                   :ref="$t('words.cv-full')"
    ...
    export default {
        ...
        methods: {
            uploadFile(e, filename) {
                this.filenameObj = filename
                this.files[this.filenameObj] = e.target.files[0];
                this.files['email'] = this.email;
            },
            submitForm() {
                this.$inertia.post('/register/instructors/uploadfiles', this.files);
                alert('submitting');
            },
        }
