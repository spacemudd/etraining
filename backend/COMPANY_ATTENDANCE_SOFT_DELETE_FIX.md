...
### 10. إصلاح مشكلة حساب أيام الحضور والغياب للمتدربين المحذوفين
- تم ضبط `start_date` و `end_date` بـ `null` للمتدربين المحذوفين في السجل الأصلي
- تم التأكد من أن المتدربين المحذوفين يعاملون مثل المتدربين العاديين في حساب الأيام
- تم إضافة فحص إضافي في ملف العرض لضمان ضبط التواريخ للمتدربين المحذوفين
- تم إضافة logging لتتبع عملية ضبط التواريخ

### 11. إصلاح مشكلة إنشاء التقارير الجديدة للمتدربين المحذوفين
- تم ضبط `start_date` و `end_date` بـ `null` للمتدربين المحذوفين في `newReport` و `clone` methods
- تم التأكد من أن المتدربين المحذوفين يعاملون مثل المتدربين العاديين عند إنشاء التقارير الجديدة
- تم إضافة logging لتتبع عملية إنشاء التقارير الجديدة

### 12. إصلاح البيانات الموجودة للمتدربين المحذوفين
- تم إنشاء console command جديد `FixDeletedTraineesAttendanceCommand`
- يقوم الأمر بالبحث عن جميع سجلات المتدربين المحذوفين في `company_attendance_reports_trainees`
- يضبط `start_date` و `end_date` بـ `null` للسجلات الموجودة
- يمكن تشغيله باستخدام `php artisan fix:deleted-trainees-attendance`
...
15. ✅ سيظهر للمتدربين المحذوفين نفس عدد أيام الحضور والغياب مثل المتدربين العاديين
16. ✅ سيتم إنشاء التقارير الجديدة بشكل صحيح للمتدربين المحذوفين
17. ✅ سيتم إصلاح البيانات الموجودة للمتدربين المحذوفين في التقارير السابقة
...
14. التحقق من أن المتدربين المحذوفين يظهر لهم نفس عدد أيام الحضور والغياب مثل المتدربين العاديين
15. التحقق من إنشاء التقارير الجديدة بشكل صحيح للمتدربين المحذوفين
16. تشغيل أمر `php artisan fix:deleted-trainees-attendance` لإصلاح البيانات الموجودة
...
- تم إصلاح مشكلة حساب أيام الحضور والغياب للمتدربين المحذوفين
- تم إصلاح مشكلة إنشاء التقارير الجديدة للمتدربين المحذوفين
- تم إنشاء أمر console لإصلاح البيانات الموجودة
