...
class CompanyTraineeActivityLogExport implements FromView, WithEvents, WithStyles, WithColumnWidths
{
    public $request;
    public $comapny_id;
    ...
    public function view(): \Illuminate\Contracts\View\View
    {
        ...
        $records = TraineeCompanyMovement::with('company')
            ->where(function($query) use ($dates){
                $query->whereBetween('in_date', [$dates])
        ...
        if ($this->company_id) {
            $records->where('company_id', $this->company_id);
        }

        //if ($this->request->trainee_name) {
        //    $records->where('trainee_name', 'LIKE', '%'.$this->request->trainee_name.'%');
        //}
