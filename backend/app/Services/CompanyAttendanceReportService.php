...
class CompanyAttendanceReportService
{
    ...
    static public function makePdf($id)
    {
        ...
        // Use template based on user selection first, then fallback to company-specific logic
        switch ($report->template_type ?? 'default') {
            ...
            case 'mini':
                ...
                break;
            default:
                // For default template, check if this is a special company
                if (in_array($report->company->id, [
                    ...
                    // Use simplified design to avoid SSL issues for special companies
                    $view = 'pdf.company-attendance-report.special-company-simple';
                    ...
                    // Use modern design template for modern companies
                    $view = 'pdf.company-attendance-report.special-company-modern';
                } else {
                    // Use standard templates based on trainee count
        ...
        switch ($record->report->template_type ?? 'default') {
            ...
            default:
                ...
                if (in_array($record->company->id, [
                    ...
                    // Use standard individual template
